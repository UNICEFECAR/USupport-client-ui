...
echo -n "Enter locale type (block, page, modal or backdrop): "
read locale_type
...
if [ "$locale_type" != "block" ] && [ "$locale_type" != "page" ] && [ "$locale_type" != "backdrop" ] && [ "$locale_type" != "modal" ]; then
    echo "Locale type must be block, page, modal or backdrop"
    exit 1
fi
...
# Read the name of the block, backdrop, modal or page from the user input
...
# Confirm such a block, backdrop, modal or page exists
